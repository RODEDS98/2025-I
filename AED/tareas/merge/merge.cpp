...
void merge(nodo* &A,nodo* &B) {
	...
	else {cout<<"pruebax"<<endl;
	    
		r = (*q)->next;  cout<<"r: "<<r<<endl;
		q = &((*q)->next); cout<<"1 *q: "<<*q<<endl;
		*q = *p;cout<<"2 *q: "<<*q<<endl;
		//p = &r;
		//A=*q;
		cout<<"A: "<<A<<" = "<<*p <<endl;
        ...
        q = &((*q)->next); cout<<"1 *q: "<<*q<<endl;  
        p=q;  cout<<"1 *p: "<<*p<<"--- "<<(*p)->valor<<endl;
        q= &B;
        *q=r; cout<<"2 q: "<<*q<<"--- "<<(*q)->valor<<endl;
         cout<<"&r: "<<&r<<" r: "<<r<<endl<<endl;
        cout<<"2 *p: "<<*p<<"--- "<<(*p)->valor<<endl;
	...
	cout<<"while ---";
	    cout<<endl<<"(*p)->valor "<<(*p)->valor<<" " <<*p <<endl;
	    cout<<"(*q)->valor "<<(*q)->valor<<"   *q " <<*q <<" q" <<q<<endl;
	    
        if((*p)->valor < (*q)->valor) { // no es lo mismo que el anterior. aqui 
            cout<<"primer if "<<endl;  p = &((*p)->next);         // esta apuntando a un campo, compara el siguiente nodo, el apuntando 
           // p = &((*p->next));      // en cambio el anterior apuntaba al mismo nodo que analizabamos
        }
        else{
            r = *p; cout<<"r: "<<r<<endl; //guardamos la direccion del nodo almacenado en el campo ya q lo cambiaremos
            cout<<"q: "<<q<<" "<<"*q: "<<*q<<endl;
            *p = *q; 
            cout<<"*p: "<<*p<<endl;
            *q = r;
...
int main() {
	...
	cout<<"Lista original memoria: "<<endl;
	print2(head1);
	print2(head2);
