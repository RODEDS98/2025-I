...
void print3(int(* A)[10], int fil, int* dir_A=nullptr, int* dir_B=nullptr ){
    for(int(* i)[10]=A ; i < A+fil ; i++){
        for( int* j = *i ; j < *(i+1) ; j++){
            
            if(j == dir_A){          // <-- siempre mostrar A
                cout<<" A";
            }
            else if(j == dir_B){     // <-- siempre mostrar B
                cout<<" B";
            }
            else if(*j==-1){
               cout<<" X";
               ...
               cout<<" A";           // redundancia por seguridad
            }
            else if(*j==200){
               cout<<" B";
            }
        ...
        while( *i != dir_B ) {
            ...
            if( j+1  < *(i+1)   ){ cout<<"derecha " <<marca <<endl;    //derecha
                ...
                if(*(j+1) == 0){
                    ...
                    cout <<"terminado"<<endl;
                    *j = marca;       // <-- marcar la celda actual
                    *(j+1) = marca;   // <-- marcar B como parte del camino
                    break;
                }
