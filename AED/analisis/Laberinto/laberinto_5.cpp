...
void reversa(int (&A)[fil][col],int(* dir_i)[col] ,int* &dir_j,int marca ){
    ...
    while( true ) {    // marca == marca+1
            ...
            else if( j+1  < *(i+1) && *(j+1) == marca ){ // abajo
                *j = (*j)*(-1);
                i++;
                j = j+col;
                continue;
            ...
            else if(  (i+1) < A+fil+1 && *(j+col) == marca ){  // derecha
                *j = (*j)*(-1);
                j--;
                continue;
            }
...
void solucion(int(& A)[fil][col], int* dir_A , int* dir_B ){             //   100 101 102 103
    ...
    if (dir_A < dir_B){
        ...
        while( *i != dir_B ) {
            ...
            if( j+1  < *(i+1) && *(j+1) != -1 ){     //derecha
                contador++;
                direccion=4;
            }  
            
            if(  (i+1) < A+fil+1 && *(j+col) != -1 ){   //abajo
                contador++;
                direccion=3;
            }
            
            if( j-1 >= *i && *(j-1) != -1 ){     //izquierda
                contador++;
                direccion=2;
            }
